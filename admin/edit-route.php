...
if (isset($_POST['update_now'])) {
    ...
    if (empty($errors)) {
        ...
        try {
            ...
            $query = 'UPDATE routes SET ' . implode('=?,', array_keys($_INP)) . '=? WHERE id=?';

            $stmt = $db->prepare($query);

            // dbg(count(array_fill(0, count($_INP), '?')));
            $stmt->bind_param(str_repeat('s', count($_INP)) . 'i', ...[...array_values($_INP), $id]);
    ...
    $_INP = $stmt->get_result()->fetch_assoc();
}
...
<div class="" style="max-width: 1320px; margin: auto;">
    <div id="main" class="col-10 mt-5">
        ...
        <form method="POST">
            ...
            <button name="update_now" class="btn btn-success d-block w-100 mb-5" type="submit">Insert Now</button>
        </form>
